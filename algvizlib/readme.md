...
+ 使用不同颜色的tracer来标记表格中被访问过的单元格。
    + \[TODO\]考虑tracer的颜色混合策略。参考资料：[rgb颜色混合](https://www.jianshu.com/p/6d9a3f39bb53)
...
+ 数组支持的操作：
    ...
    + 交换数组中的两个位置中的元素值：
        + 使用swap(tracer1, tracer2)接口进行交换调用，交换后只改变元素内容，不改变tracer的索引。
        + 交换时自动产生动画，如果是直方图形式则沿线性运动，如果非直方图形式则沿贝塞尔曲线运动。
    + 求数组长度__len()\_\_接口重载。
+ 动画刷新时的步骤：
    1. 淡出要删除的元素。
    2. 将剩余元素移动到最终位置。
